...
def move_target_to_end(dataframe, target_column):
    # Verificar si la columna target existe en el DataFrame
    if target_column not in dataframe.columns:
        print(f"La columna {target_column} no existe en el DataFrame.")
        return dataframe

    # Obtener la lista de columnas excluyendo la columna target
    other_columns = [col for col in dataframe.columns if col != target_column]

    # Reorganizar las columnas colocando la columna target al final
    reordered_columns = other_columns + [target_column]

    # Crear un nuevo DataFrame con las columnas reorganizadas
    new_dataframe = dataframe[reordered_columns]
...
def small_histogram(dataset, target):
    ...
    num_variables = len(numeric_variables) - 1  # Excluir la variable objetivo
    num_rows = (num_variables - 1) // 3 + 1  # Calcular el número de filas necesarias
    num_cols = min(num_variables, 3)  # No más de 3 columnas
    ...
    for i, variable in enumerate(numeric_variables):
        ...
        # Calcular la posición de la subgráfica actual
        row = i // num_cols
        col = i % num_cols
        
        sns.histplot(dataset[variable], bins=20, kde=True, ax=axes[row, col])
